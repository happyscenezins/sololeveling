...
function loadFloor(floorIdx) {
    ...
    loadChallenge(floorData.challenge);
}
...
function executeSpell(selectedIndex) {
    ...
    if (selectedIndex === q.correctIndex) {
        ...
        battleLog.innerHTML = `<strong>${isCrit ? "⚡ SHADOW CRITICAL HIT!" : "DIRECT HIT!"}</strong> Dealt ${damage} damage. <em>${q.explanation}</em>`;
        ...
        battleLog.innerHTML = `<strong>SPELL COLLAPSED!</strong> Grammatical divergence detected. Try again.`;
        setTimeout(() => buttons.forEach(b => b.disabled = false), 1000);
    }
}

function handleBossDefeat() {
    alert(`🏆 GATE CLEARED! Shadow Extracted: ${dungeonFloors[gameState.currentFloor].boss.name}!`);
...
function checkLevelUp() {
    while (gameState.exp >= gameState.expToNext) {
        ...
        gameState.level += 1;
        gameState.baseAtk += 20; // Drastic power scaling
        gameState.expToNext = Math.floor(gameState.expToNext * 1.4);
        alert(`⚡ [SYSTEM NOTIFICATION: LEVEL UP] You have reached Level ${gameState.level}! Base ATK increased to ${gameState.baseAtk}.`);
    }
}
...
window.buyItem = function (itemId) {
    ...
    if (gameState.gold >= item.price) {
        ...
        alert("❌ Insufficient Gold! Clear more Gate Bosses.");
    }
};
...
resetSystemBtn.onclick = () => {
    if (confirm("⚠️ System Reset: Re-awaken as Level 1 Hunter and reset all artifacts?")) {
